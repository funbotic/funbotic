...
class Funbotic_Admin {
	...
	public function enqueue_styles() {
		 ...
		 * defined in Funbotic_Loader as all of the hooks are defined
		 * in that particular class.
		 *
		 * The Funbotic_Loader will then create the relationship
		 * between the defined hooks and the functions defined in this
		 * class.
		 */

		wp_enqueue_style( $this->plugin_name, plugin_dir_url( __FILE__ ) . 'css/funbotic-admin.css', array(), $this->version, 'all' );

		if ( 'settings_page_funbotic' == get_current_screen() -> id ) {
			// CSS stylesheet for Color Picker.
			wp_enqueue_style( 'wp-color-picker' );
			wp_enqueue_style( $this->plugin_name, plugin_dir_url( __FILE__ ) . 'css/funbotic-admin.css', array( 'wp-color-picker' ), $this->version, 'all' );
		}
	...
	public function enqueue_scripts() {
		...
		wp_enqueue_script( $this->plugin_name, plugin_dir_url( __FILE__ ) . 'js/funbotic-admin.js', array( 'jquery' ), $this->version, false );

		if ( 'settings_page_funbotic' == get_current_screen() -> id ) {
			wp_enqueue_media();
			wp_enqueue_script( $this->plugin_name, plugin_dir_url( __FILE__ ) . 'js/funbotic.js', array( 'jquery', 'wp-color-picker' ), $this->version, false );
		}
	...
	*/
	public function add_plugin_admin_menu() {
		...
		*
		*/
		add_options_page( 'Funbotic Options', 'Funbotic', 'manage_options', $this->plugin_name, array($this, 'display_plugin_setup_page') );
	}
	...
	*/
	public function display_plugin_setup_page() {
		include_once( 'partials/funbotic-admin-display.php' );
	}
     ...
     * @since    1.1.6
     */
	private function funbotic_login_logo_css() {
		if ( isset( $this->funbotic_options['login_logo_id'] ) && ! empty( $this->funbotic_options['login_logo_id'] ) ) {
			$login_logo = wp_get_attachment_image_src( $this->funbotic_options['login_logo_id'], 'thumbnail' );
	...
	private function funbotic_login_background_color(){
		if ( isset( $this->funbotic_options['login_background_color']) && ! empty( $this->funbotic_options['login_background_color'] ) ) {
			$background_color_css  = "body.login{ background:" . $this->funbotic_options['login_background_color'] . "!important;}";
			return $background_color_css;
		}
	}

	// Get Button and links color is set and different from #00A0D2 return it's css
	private function funbotic_login_button_color(){
		if ( isset( $this->funbotic_options['login_button_primary_color'] ) && ! empty( $this->funbotic_options['login_button_primary_color'] ) ) {
			$button_color = $this->funbotic_options['login_button_primary_color'];
	...
	public function funbotic_login_css(){
		if ( ! empty( $this->funbotic_options['login_logo_id'] ) || $this->funbotic_login_background_color() != null || $this->funbotic_login_button_color() != null){
			echo '<style>';
			if ( ! empty( $this->funbotic_options['login_logo_id'] ) ) {
				  echo $this->funbotic_login_logo_css();
			}
			if ( $this->funbotic_login_background_color() != null ){
				  echo $this->funbotic_login_background_color();
			}
			if ( $this->funbotic_login_button_color() != null ) {
				  echo $this->funbotic_login_button_color();
			}
			echo '</style>';
		}
	}
